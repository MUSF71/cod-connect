...
next-app's `src/components/` (under `/login` and `/cadastro`, see `src/app/login/page.tsx` and `src/app/cadastro/page.tsx`) is the reference implementation of this pattern end-to-end — atoms (`Button`, `Input`, `Checkbox`, `Label`, `Divider`, `TextLink`, icon atoms) compose into molecules (`FormField`, `RememberForgot`, `SocialButton`, `SocialLogins`), which compose into organisms (`AuthBanner`, `LoginForm`, `SignupForm`), assembled by the shared `AuthLayout` template. `/login` and `/cadastro` reuse the same atoms/molecules/template and only diverge in their organism (`LoginForm` vs `SignupForm`) — follow that shape (props interface, colocated test, Tailwind classes only, maximize reuse of existing atoms/molecules) for new auth-adjacent screens in either app.
